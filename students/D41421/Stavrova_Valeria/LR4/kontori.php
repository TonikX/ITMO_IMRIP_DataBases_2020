...
<!DOCTYPE html>
	...
	<body>
		...
		<h3>Редактировать</h3>
		<ul>
			<form name="insert" action="kontori.php" method="POST" >
				<ul>Номер конторы:</ul><ul><input type="int" name="Номер_конторы"/></ul>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if (isset($_POST["Add"])) {
            $infoA = "insert into $ClientTable values ('$_POST[Номер_конторы]', '$_POST[Название]')";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if (isset($_POST["Delete"])) {
            $infoA = "delete from $ClientTable where \"Номер_конторы\"='$_POST[Номер_конторы]'";
            $result = pg_query($db, $infoA);
			$result = pg_fetch_all($result);
			pg_close($db);
         }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if (isset($_POST["upd"])) {
            $infoA = "update $ClientTable set \"Название\"='$_POST[Название]' where \"Номер_конторы\"='$_POST[Номер_конторы]'";
            $result = pg_query($db, $infoA);
			$result = pg_fetch_all($result);
			pg_close($db);
         }
